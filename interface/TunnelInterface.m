...
#define kTunnelInterfaceErrorDomain @"com.oclemon.soVpn.TunnelInterface"
...
@property (nonatomic, strong) id udpSocket;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        Class socketClass = NSClassFromString(@"GCDAsyncUdpSocket");
        if (socketClass) {
            _udpSocket = [[socketClass alloc] initWithDelegate:self 
                                               delegateQueue:dispatch_queue_create("udp", NULL)];
        }
    }
    return self;
}
...
+ (NSError *)setupWithPacketTunnelFlow:(NEPacketTunnelFlow *)packetFlow {
    ...
    NSError *error;
    id udpSocket = [TunnelInterface sharedInterface].udpSocket;
    if (!udpSocket) {
        // Handle case where GCDAsyncUdpSocket is not available
    ...
    // Use performSelector to avoid direct references
    if ([udpSocket respondsToSelector:@selector(bindToPort:error:)]) {
        if (![udpSocket performSelector:@selector(bindToPort:error:) 
                           withObject:@0 
                           withObject:&error]) {
            return error;
        }
...
+ (void)startTun2Socks: (int)socksServerPort udp:(int)udpPort{
    [NSThread detachNewThreadSelector:@selector(_startTun2Socks:) toTarget:[TunnelInterface sharedInterface] withObject:@[@(socksServerPort),@(udpPort)]];
}
...
- (void)_startTun2Socks: (NSArray *)ports {
    NSNumber*socksServerPort = ports[0];
    char socks_server[50];
//    sprintf(socks_server, "192.168.31.247:6153");
    sprintf(socks_server, "127.0.0.1:%d", (int)([socksServerPort integerValue]));
    char udp_server[50];
    sprintf(udp_server, "127.0.0.1:%d", (int)([ports[1] integerValue]));
    NSLog(@"ss tcp:%s",socks_server);
    NSLog(@"ss udp:%s",udp_server);
...
- (void)handleUDPPacketv2: (NSData *)packet {
    uint8_t message[1492+2];
    memcpy(message + 2, packet.bytes, packet.length);
    message[0] = packet.length / 256;
    message[1] = packet.length % 256;
    write(self.writeFd , message , packet.length + 2);
}
- (void)handleUDPPacket: (NSData *)packet {//send udp packages received from tunnel to remote server
    uint8_t *data = (uint8_t *)packet.bytes;
    int data_len = (int)packet.length;
    struct ip_hdr *iphdr = (struct ip_hdr *)data;
    uint8_t version = IPH_V(iphdr);
...
//            NSLog(@"udp to:%@ port:%d",destHost,ntohs(udphdr->dest));
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext {//received udp data from remote ,send it to tunnel;
